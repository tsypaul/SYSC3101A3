...
(define (quote-checker type)
  (lambda (exp)
    (if (pair? (cdr exp))
        (error "Please enter a quote with only one argument")
    (and (pair? exp) (eq? (car exp) type)))))

(define quote-exp? (quote-checker 'quote))
